...
const Footer = () => {
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
    <div className="flex flex-col md:flex-row md:justify-between items-center gap-4 md:gap-6">
      ...
      <form className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
        ...
        <button
          type="submit"
          className="bg-orange-500 hover:bg-orange-600 text-white px-4 sm:px-5 py-2 rounded-md w-full sm:w-auto"
        >
          {t("SEND")}
        </button>
      </form>
    </div>
    <p className="text-xs text-gray-400 mt-4 sm:mt-2 text-center md:text-right">
  ...
  {/* Footer Links */}
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-6 md:gap-8">
    {footerData.map((section) => (
      <div key={section.title}>
        <h4 className="font-semibold mb-2 text-base md:text-sm">{t(section.title)}</h4>
          ...
          {section.links.map((link, index) => {
            ...
            return (
              <li key={index}>
                <Link
                  href={linkHref}
                  className="hover:underline cursor-pointer text-gray-500 block text-sm"
                >
                  {hasPrice ? (
                    <span>
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 flex flex-col sm:flex-row justify-between items-center text-xs text-gray-500 gap-4 sm:gap-2">
    ...
    <div className="relative group flex flex-col items-center justify-center">
      <div className="flex items-center gap-3 sm:gap-4 transition-all duration-300 group-hover:opacity-20 group-hover:blur-[1px] flex-wrap justify-center">
        ...
        <img
          src="/stc-pay.png"
          className="h-7 sm:h-8 grayscale w-auto object-contain"
          alt="STC Pay"
        />
        <img
    ...
    <div className="flex gap-3 sm:gap-4 text-gray-500 flex-wrap justify-center sm:justify-start">
      <Link href="/privacy" className="hover:underline">{t("Terms and Conditions")}</Link>
      <Link href="/privacy" className="hover:underline">{t("Privacy Policy")}</Link>
    </div>

    {/* Social Icons */}
    <div className="flex gap-3 sm:gap-4 text-gray-500 mt-2 sm:mt-0 justify-center sm:justify-start">
