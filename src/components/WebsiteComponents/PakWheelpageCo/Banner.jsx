...
const Banner = () => {
  ...
  const cityData = homeData?.usedCars?.cities || [];

  // Convert everything to string safely
  const formattedCityData = cityData.map((c) => {
  ...
  // Merge & dedupe
  const allCities = [...new Set([...citiesList, ...formattedCityData])].sort();

  const popularCities = ["Karachi", "Lahore", "Islamabad", "Rawalpindi", "Peshawar"];
  const otherCities = allCities.filter((c) => !popularCities.includes(c));
  ...
  const handleSearch = (query) => {
    ...
    console.log("Navigating to:", `motors?${queryString}`);

    router.push(`motors?${queryString}`);
  }
  ...
  return (
    ...
    >
      <div className="w-full max-w-6xl px-4 sm:px-6 lg:px-8 text-center text-white">
        ...
        <div
          ref={wrapperRef}
          className="relative bg-white rounded-lg sm:rounded-md shadow-xl max-w-4xl mx-auto flex flex-col md:flex-row items-stretch divide-y md:divide-y-0 md:divide-x"
        >
          {/* Make / Model Input */}
          <div className="flex-1 relative ">
            <input
              ...
              className="w-full px-5 py-4 text-gray-700 border-2 border-gray-300 outline-none placeholder-gray-400 text-base rounded-t-lg md:rounded-l-lg md:rounded-tr-none"
            />
          </div>
          ...
          >
            ...
            {cityOpen && (
              <div
                className="absolute top-full left-0 w-full md:w-[280px] shadow-2xl z-[100] text-left border border-gray-200 mt-1 rounded-md overflow-hidden bg-white"
          ...
          <button onClick={handleSearch}
            className="bg-green-600 hover:bg-green-700 text-white px-6 py-5 md:py-0 md:px-8 flex items-center justify-center transition-colors font-medium text-base md:text-lg">
            <svg
              width="22"
              height="22"
              fill="none"
