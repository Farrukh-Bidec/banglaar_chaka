...
const CarsforSaleView = () => {
  ...
  const [windowWidth, setWindowWidth] = useState(0); // Client-only width
  ...
  const nextSlide = () => {
    const maxIndex = featuredCars.length - (windowWidth < 768 ? 1 : windowWidth < 1024 ? 2 : 4);
  ...
  // Slide width in % depending on viewport
  const slideWidth = windowWidth < 640 ? 100 : 280;
    ...
    <div className="py-8 sm:py-10 px-4 sm:px-6 lg:px-12 xl:px-20 flex justify-center bg-gray-50">
      <div className="max-w-7xl w-full">
        ...
        <div className="relative group overflow-visible">
          ...
          <button
            onClick={nextSlide}
            disabled={currentIndex >= featuredCars.length - (windowWidth < 768 ? 1 : windowWidth < 1024 ? 2 : 4)}
            className={`hidden sm:flex absolute right-0 md:right-[-20px] top-1/2 -translate-y-1/2 z-10 bg-white w-9 h-9 md:w-10 md:h-10 rounded-full items-center justify-center shadow-lg border border-gray-200 transition-all duration-200 ${
              currentIndex >= featuredCars.length - (windowWidth < 768 ? 1 : windowWidth < 1024 ? 2 : 4) 
